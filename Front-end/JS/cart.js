...
function deleteProduct(itemId){
  ...
  if (localStorage.getItem('products')) {
    ...
    alert('Le produit va être supprimé de votre panier');
...
function checkFormAndPostRequest() {
  ...
  var mailFormat = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
    if (
    ...
    }else if(!inputMail.value.match(mailFormat)){
      e.preventDefault();
      erreur.innerText = "Votre adresse email n'est pas valide";
      ...
      let productsBought = [];
      for(let copyProduct of copyOfLS){
        productsBought.push(copyProduct['_id']); // Pas les produits achetés mais les ids des produits achetés
      }
